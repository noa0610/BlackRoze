...
namespace BlackRose.Core.Models.Units
{
    ...
    public class Enemy_Shooter : GroundedUnit
    {
        ...
        protected override void RegisterStats()
        {
            ...
            _stateMachine.AddState(States.dead, new Idle());
                ...
                private void SearchPlayer()
                {
                        ...
                        if (IsMatchingState(States.move) && _searchAssistance.Execute("red", list, out var ui))
                        {
        ...
        private bool IsMatchingStates(params States[] states)
        {
            foreach (var state in states)
            {
                if (IsMatchingState(state)) return true;
            }
            return false;
        }
    }
