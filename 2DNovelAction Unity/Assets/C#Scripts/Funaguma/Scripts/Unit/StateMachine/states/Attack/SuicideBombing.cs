...
namespace BlackRose
{
    public class SuicideBombing : StateWithAnime
    {
        ...
        public SuicideBombing(SphereCollider collider)
        {
        ...
        private async UniTask Explode(UnitBase parent)
        {
            ...
            var hitColliders = Physics.OverlapSphere(_collider.transform.position, _radius);
            foreach (var hitCollider in hitColliders)
            {
                ...
                if (hitCollider.TryGetComponent<UnitBase>(out var unit))
                {
                    ...
                    unit.StatusManager.TakeDamage(_damage == -5 ? 1 : _damage);
                }
            }
